...
void fileMerger::CopyDir(TDirectory *source, Int_t ilevel) {
  ...
  {
		//std::cout << "--- I have to mkdir now... maybe here it goes all wrong ---" << std::endl ;
		//std::cout << "--- What's your name?     source->GetName(): " << source->GetName() << std::endl ;
    ...
    if (cl->InheritsFrom(TDirectory::Class())) {
      ...
      if(dirname != "events"){
        ...
        CopyDir(subdir, MAKE_NEW_FOLDER);
				//std::cout << "--- After making a new folder. ---" << std::endl ;
        adir->cd();
      }
...
void fileMerger::CleanUp(){
  ...
  for(int i=0; i<(int)auxfiles.size(); i++){
    std::string auxfile = auxfiles[i];
    std::cout << "fileMerger::CleanUp()          removing file:  "<<auxfile.c_str()<<std::endl;
